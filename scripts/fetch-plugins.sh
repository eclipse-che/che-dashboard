...
#                    When unset, all plugins found in the release are installed.
#
...
# Resolve the list of plugins to install.
# When PLUGINS_LIST is unset/empty, discover all plugins from the release assets.
if [[ -z "${PLUGINS_LIST:-}" ]]; then
  ...
  echo "[INFO] PLUGINS_LIST not set — discovering plugins from release: ${PLUGINS_REF}"
  # Use read loop instead of mapfile for POSIX sh / Alpine sh compatibility
  while IFS= read -r line; do PLUGINS+=("$line"); done < <(curl -fsSL "$API_URL" | jq -r '.assets[].name | select(endswith(".zip")) | sub("\\.zip$"; "")')
  if [[ ${#PLUGINS[@]} -eq 0 ]]; then
    echo "[ERROR] No plugin ZIPs found in release ${PLUGINS_REF} of ${PLUGINS_REPO}"
    exit 1
  fi
  echo "[INFO] Discovered plugins: ${PLUGINS[*]}"
else
  mapfile -t PLUGINS < <(echo "$PLUGINS_LIST" | jq -r '.[]')
fi
