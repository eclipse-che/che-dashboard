...
export function registerFrontendPlugin(plugin: FrontendPlugin): void {
  ...
  if (frontendPlugins.some(p => p.manifest.id === plugin.manifest.id)) {
    console.warn(
      `[PluginSystem] Plugin "${plugin.manifest.id}" is already registered. Skipping duplicate registration.`,
    );
    return;
  }
