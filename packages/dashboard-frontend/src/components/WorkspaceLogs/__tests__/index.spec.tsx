...
import { podLogsActionCreators, PodLogsState } from '@/store/Pods/Logs';
...
jest.mock('@/store/Pods/Logs', () => ({
  ...jest.requireActual('@/store/Pods/Logs'),
  podLogsActionCreators: {
    watchPodLogs:
      (pod: V1Pod): AppThunk =>
      async (): Promise<void> =>
  ...
  } as typeof podLogsActionCreators,
}));
...
describe('The WorkspaceLogs component', () => {
  ...
  describe('start watching logs', () => {
    ...
    it('should stop and then start watching logs', async () => {
      ...
      const nextNextStore = new MockStoreBuilder()
        .withDevWorkspaces({
          workspaces: [devWorkspace],
        })
        .withPods({
