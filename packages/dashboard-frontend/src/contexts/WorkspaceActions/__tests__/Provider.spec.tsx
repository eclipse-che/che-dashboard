...
describe('WorkspaceActionsProvider', () => {
  ...
  describe('handle actions', () => {
    ...
    describe('SCC mismatch warning', () => {
      ...
      test('start in background with matching SCC does not log warning', async () => {
        // Generated by GPT-5.2
        console.warn = jest.fn();
        mockStartWorkspace.mockResolvedValueOnce(undefined);

        const storeWithMatchingScc = new MockStoreBuilder()
          .withDevWorkspaces({
            workspaces: [
              new DevWorkspaceBuilder()
                .withName('wksp-' + wantDelete[0])
                ...
                .withUID(wantDelete[0])
                .withTemplateAttributes({
                  [DEVWORKSPACE_CONTAINER_SCC_ATTR]: 'container-run', // Matches server SCC
                })
                .build(),
            ],
          })
        ...
        renderComponent(storeWithMatchingScc, WorkspaceAction.START_IN_BACKGROUND, wantDelete[0]);
