...
describe('for DevWorkspace', () => {
  ...
  it('should set name by updating the "kubernetes.io/metadata.name" label', () => {
    const name = 'wksp-1234';
    const devWorkspace = new DevWorkspaceBuilder().withName(name).build();
    const workspace = constructWorkspace(devWorkspace);
    ...
    expect(workspace.name).toEqual('renamed-workspace');
    expect(devWorkspace.metadata.labels?.[DEVWORKSPACE_LABEL_METADATA_NAME]).toEqual(
      'renamed-workspace',
    );
  });

  it('should set name when labels are initially absent', () => {
    const devWorkspace = new DevWorkspaceBuilder().withName('wksp-1234').build();
    delete devWorkspace.metadata.labels;
    const workspace = constructWorkspace(devWorkspace);
  ...
  describe('source', () => {
    ...
    describe('factory params handling', () => {
      it('should return just the factory url without appending other factory attrs', () => {
        // Factory attrs like che-editor and storageType must NOT appear in source —
        // they would produce a wrong "Git repo URL" in the Workspace Details UI.
        const devWorkspace = new DevWorkspaceBuilder()
          .withMetadata({
            name: 'test-workspace',
            annotations: {
              [DEVWORKSPACE_DEVFILE_SOURCE]: dump({
                factory: {
                  params:
                    'che-editor=che-incubator/che-code/latest&storageType=per-workspace&url=http://localhost/api/airgap-sample/devfile/download?id=java-lombok',
                },
              }),
            },
          })
        ...
        expect(workspace.source).toEqual(
          'http://localhost/api/airgap-sample/devfile/download?id=java-lombok',
        );
        expect(workspace.source).not.toContain('che-editor');
        expect(workspace.source).not.toContain('storageType');
      });

      it('should decode %3F and %3D in airgap sample URLs so source matches factoryParams.sourceUrl', () => {
        // workspace.source reads from the DevWorkspace annotation via URLSearchParams.get(),
        // which decodes %3F → ? and %3D → =.
        // buildFactoryParams.sourceUrl also applies decodeURIComponent for the same result.
        // Both sides must therefore produce the same decoded string for comparison to work.
        const devWorkspace = new DevWorkspaceBuilder()
          .withMetadata({
            name: 'test-workspace',
            annotations: {
              [DEVWORKSPACE_DEVFILE_SOURCE]: dump({
                factory: {
                  params:
                    'url=http://che-dashboard.eclipse-che.svc:8080/dashboard/api/airgap-sample/devfile/download%3Fid%3Dnodejs-express&che-editor=che-incubator/che-code/latest&storageType=per-user',
                },
              }),
            },
          })
          .build();
        const workspace = constructWorkspace(devWorkspace);
        // URLSearchParams.get() decodes %3F → ? and %3D → =
        expect(workspace.source).toEqual(
      ...
      it('should handle url without extra factory params', () => {
        const devWorkspace = new DevWorkspaceBuilder()
          .withMetadata({
            name: 'test-workspace',
            annotations: {
          ...
          .build();
        const workspace = constructWorkspace(devWorkspace);
        expect(workspace.source).toEqual('https://github.com/test/repo');
      });
    });
    ...
    describe('SSH location revision handling', () => {
        ...
        const devWorkspace = new DevWorkspaceBuilder()
          .withMetadata({
            ...
            annotations: {
              [DEVWORKSPACE_DEVFILE_SOURCE]: dump({
                factory: {
                  params: 'url=git@github.com:user/repo.git',
                },
              }),
            },
          })
          ...
          .build();
        const workspace = constructWorkspace(devWorkspace);
        expect(workspace.source).toEqual('git@github.com:user/repo.git?revision=feature-branch');
      });
      ...
      it('should not extract revision for SSH if already in params', () => {
        const devWorkspace = new DevWorkspaceBuilder()
          .withMetadata({
            ...
            annotations: {
              [DEVWORKSPACE_DEVFILE_SOURCE]: dump({
                factory: {
                  params: 'url=git@github.com:user/repo.git&revision=main',
                },
              }),
            },
          })
        ...
        expect(workspace.source).toEqual('git@github.com:user/repo.git?revision=main');
      });
      ...
      it('should not extract revision from git projects for HTTP location', () => {
        ...
        expect(workspace.source).toEqual('https://github.com/user/repo.git');
      });
      ...
      it('should handle SSH location without projects', () => {
        const devWorkspace = new DevWorkspaceBuilder()
          ...
          .build();
        const workspace = constructWorkspace(devWorkspace);
        // No projects and no revision in params, so no revision is appended.
        // storageType is NOT included — source is just the SSH URL.
        expect(workspace.source).toEqual('ssh://git@bitbucket.org:user/repo.git');
      });
