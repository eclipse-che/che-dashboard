...
import { ExternalLinkSquareAltIcon, PencilAltIcon } from '@patternfly/react-icons';
...
class WorkspaceNameFormGroup extends React.PureComponent<Props, State> {
  ...
  private handleNameChange(_event: React.FormEvent, editedName: string): void {
  ...
  private getHelperText(): React.ReactNode {
    const { validated, errorMessage, errorTooltipMessage } = this.state;

    if (validated !== ValidatedOptions.error || !errorMessage) {
      return null;
    }

    const content = errorTooltipMessage ? (
    ...
    return (
      <FormHelperText>
        <HelperText>
          <HelperTextItem variant="error">{content}</HelperTextItem>
        </HelperText>
      </FormHelperText>
    );
  }
  ...
  private getEditModal(): React.ReactNode {
    ...
    return (
      ...
      >
        <ModalHeader title="Edit Workspace Name" />
        <ModalBody>
          <FormGroup fieldId="edit-workspace-name" className={styles.inputFormGroup}>
            <TextInput
  ...
  public render(): React.ReactNode {
    ...
    return (
      <FormGroup label="Workspace" fieldId="workspace-name">
        {readonly ? (
